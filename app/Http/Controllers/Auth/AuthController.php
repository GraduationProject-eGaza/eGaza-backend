...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        $validated = $request->validate([
            ...
            'username' => 'required_if:type,citizen,government-institute,government-employee',


            // Citizen & Gov Employee Fields
            ...
            'governorate' => 'required_if:type,citizen,government-employee,government-institute',
            'city' => 'required_if:type,citizen,government-employee,government-institute',
            'street' => 'required_if:type,citizen,government-employee,government-institute',
            'nearest_landmark' => 'required_if:type,citizen,government-employee',
        ...
        switch ($validated['type']) {
            ...
            case 'government-employee':
                // Validate government_id by checking if the institute exists
                $institute = User::where('type', 'government-institute')
                    ->where('government_id', $validated['government_id'])
                    ->first();

                if (!$institute) {
                    return response()->json([
                        'message' => 'Invalid government ID. No matching institute found.'
                    ], 422);
                }
                ...
                $userData += [
                    ...
                    'government_id' => $validated['government_id'], // optional but you can keep for reference
    ...
    public function verifyEmail(Request $request)
    {
        ...
        $userQuery = User::where('type', $request->type);

        if ($request->type === 'government-institute') {
            $userQuery->where('representative_national_id', $request->representative_national_id);
        } else {
            $userQuery->where('national_id', $request->national_id);
        }
    ...
    public function forgetPassword(Request $request)
    {
        ...
        try {
            ...
            return response()->json(
                [
                    'massege' => 'Password reset token sent successfully.',
                    'email' => $user->email
                ],
                200
            );
